...
export class AcquisitionManager {
  ...
  public queryUpdateWithCurrentPackage(
    ...
    if (!currentPackage || !currentPackage.appVersion) {
      throw new CodePushPackageError(
      ...
      switch (status) {
        ...
        default:
          if (callback) {
            if (!status) {
              callback(
                new CodePushDeployStatusError("Missing status argument."),
                /*not used*/ null
              );
            } else {
              callback(
                new CodePushDeployStatusError(
    ...
    this._httpRequester.request(
      ...
      (error: Error, response: Http.Response): void => {
        if (callback) {
          ...
          if (response.statusCode !== 200) {
            callback(
              new CodePushHttpError(response.statusCode + ": " + response.body),
              /*not used*/ null
            );
            return;
          }
